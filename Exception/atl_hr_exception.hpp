...
namespace is
{
    namespace com
    {
        /* Windows-ATLに関する例外スロー */
#define IS_HR_ERROR(hr, msg, ...)                                       \
    throw HrException(hr, is::common::FormatString(msg, ##__VA_ARGS__), \
                      __func__, __FILE__, __LINE__);

#define IS_HR_CHECK(hr, msg, ...)                                               \
    if (FAILED(hr))                                                             \
    {                                                                           \
        IS_HR_ERROR(hr, std::string("Failed `" #hr "`: ") + msg, ##__VA_ARGS__) \
    }

#define IS_HR_FORCE_ASSERT(hr, msg, ...)                                          \
    if (FAILED(hr))                                                               \
    {                                                                             \
        ...
        inline ATL::CString GetAtlHrErrorString(HRESULT hr)
        {
            /* ShiftJisでメッセージ作成 */
        ...
        /**
         * @brief HRESULT HANDLE Exception クラス
         *
         */
        class IS_WIN_API AtlHrException : public std::exception
        {
        protected:
        ...
        public:
            ...
            virtual ~WinHrException() noexcept {}
            virtual LPTCSTR What() const noexcept { return mFullMsg.GetString(); }
            handler::Result GetError() const { return mResult; }
        };
    }
}